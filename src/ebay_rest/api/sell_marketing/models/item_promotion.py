...
class ItemPromotion(object):
    ...
    def apply_discount_to_single_item_only(self):
        ...
        This flag is relevant in only when <b>promotionType</b> is set to <code>VOLUME_DISCOUNT</code>. For details on volume pricing discounts, see <a href=\"/api-docs/sell/static/marketing/pm-volume-discounts.html\">Configuring volume pricing discounts</a>.  <br><br>If set to <code>true</code>, the discount is applied only when the buyer purchases multiple quantities of a single item that is being discounted. Otherwise, the discount applies to multiple quantities of any the items being discounted. Different variations of a multi-variation item are considered to be the same item. Note that this flag is not relevant if the <b>inventoryCriterion</b> container identifies a single listing ID for the discount.  # noqa: E501
    ...
    def name(self):
        ...
        The seller-defined name or \"title\" of the discount that the seller can use to identify a discounts. This label is not displayed in end-user flows.  <br><br><b>Maximum length:</b> 90  # noqa: E501
    ...
    def promotion_status(self):
        ...
        The current status of the discount. When creating a new discount, this value must be set to either <code>DRAFT</code> or <code>SCHEDULED</code>.  <br><br>Note that you must set this value to <code>SCHEDULED</code> when you update a <b>RUNNING</b> discount. For implementation help, refer to <a href='https://developer.ebay.com/api-docs/sell/marketing/types/sme:PromotionStatusEnum'>eBay API documentation</a>  # noqa: E501
