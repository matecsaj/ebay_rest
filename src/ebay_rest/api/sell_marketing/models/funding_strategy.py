...
class FundingStrategy(object):
    ...
    def __init__(self, ad_rate_strategy=None, bidding_strategy=None, bid_percentage=None, dynamic_ad_rate_preferences=None, funding_model=None):  # noqa: E501
    ...
    def bidding_strategy(self):
        ...
        Indicates the bidding strategy for an onsite Cost Per Click (CPC) campaign.<br><br><span class=\"tablenote\"><b>Note:</b> This field is applicable only to onsite Cost Per Click (CPC) campaigns.</span><br>Valid values are:<ul><li><code>FIXED</code><br><br>When using a fixed bidding strategy, sellers manually assign and adjust keyword bids for the CPC campaign.</li><li><code>DYNAMIC</code><br><br>When using a dynamic bidding strategy, eBay manages the campaign's keyword bids and automatically updates them daily to the suggested bid.<br><br><span class=\"tablenote\"><b>Note:</b> For a CPC campaign using dynamic bidding, sellers can continue to manually add keywords for the campaign, but they are no longer able to manually adjust their associated bid values. In order to manually adjust bid values, sellers must use the <code>FIXED</code> bidding strategy.</span></li></ul><br><br><b>Default value:</b> FIXED For implementation help, refer to <a href='https://developer.ebay.com/api-docs/sell/marketing/types/pls:BiddingStrategyEnum'>eBay API documentation</a>  # noqa: E501
