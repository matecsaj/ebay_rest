...
class Transaction(object):
    ...
    def order_line_items(self):
        ...
        This array either shows the order line item transactional fees related to a <code>SALE</code> transaction and deducted from the payout associated with that order, or it shows the transactional fee credits going back to the seller in the case of a <code>REFUND</code> transaction.<br><br><span class=\"tablenote\"><strong>Note:</strong> In certain circumstances, transactional fees like <code>FINAL_VALUE_FEE</code> and <code>FINAL_VALUE_FEE_FIXED_PER_ORDER</code> are not deducted from a seller payout, but instead they are billed to the seller's account as \"non-sale charges\". When this happens, the <code>SALE</code> transaction entity will not have these fees under the <a href=\"/api-docs/sell/finances/resources/transaction/methods/getTransactions#response.transactions.orderLineItems\">orderLineItems</a> array, but they will appear as separate <code>NON_SALE_CHARGE</code> transactions. When this happens, and you want to see those transactional fees for the order, one thing you can do is make another call to <b>getTransactions</b> and filter against the <b>orderId</b>. In the response, you will see the <code>SALE</code> transaction and the <code>NON_SALE_CHARGE</code> transactions applied against the order.  # noqa: E501
