...
class API:
    ...
    def buy_browse_search(self, **kwargs):  # noqa: E501
        ...
        :param str sort: Specifies the order and the field name to use to sort the items. You can sort items by price (in ascending or descending order) or by distance (only applicable if the \"pickup\" filters are used, and only ascending order is supported). You can also sort items by listing date, with the most recently listed (newest) items appearing first.Note:  To sort in descending order, insert a hyphen (-) before the field name. If no sort parameter is submitted, the result set is sorted by "Best Match".The following are examples of using the  sort query parameter.SortResult   sort=price  Sorts by price + shipping cost in ascending order (lowest price first)This sorting option is only guaranteed to work correctly if the X-EBAY-C-ENDUSERCTX request header is used, with the contextualLocation parameter being used to set the delivery country and postal code. Below is an example of how this header would be used to do this (note the URL encoding):X-EBAY-C-ENDUSERCTX: contextualLocation=country%3DUS%2Czip%3D19406   sort=-price  Sorts by price + shipping cost in descending order (highest price first)This sorting option is only guaranteed to work correctly if the X-EBAY-C-ENDUSERCTX request header is used, with the contextualLocation parameter being used to set the delivery country and postal code. Below is an example of how this header would be used to do this (note the URL encoding):X-EBAY-C-ENDUSERCTX: contextualLocation=country%3DUS%2Czip%3D19406   sort=distance  Sorts by  distance in ascending order (shortest distance first)   sort=newlyListed Sorts by listing date (most recently listed/newest items first)   sort=endingSoonest Sorts by date/time the listing ends (listings nearest to end date/time first)   Default:  Ascending For implementation help, refer to eBay API documentation at https://developer.ebay.com/api-docs/buy/browse/types/cos:SortField
