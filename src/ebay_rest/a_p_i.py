...
class API(APIPrivate):
    ...
    def sell_compliance_get_listing_violations(self, x_ebay_c_marketplace_id, compliance_type, **kwargs):  # noqa: E501
        ...
        :param str compliance_type: Note: The ASPECTS_ADOPTION compliance type has been deprecated and will be decommissioned on August 31, 2025. This query parameter specifies the compliance type of the listing violations being retrieved. Only one compliance type value can be specified.  See ComplianceTypeEnum for more information on supported compliance types. (required)
    ...
    def sell_compliance_get_listing_violations_summary(self, x_ebay_c_marketplace_id, **kwargs):  # noqa: E501
        ...
        :param str x_ebay_c_marketplace_id: This header identifies the seller's eBay marketplace. Supported values for this header can be found in the MarketplaceIdEnum type definition. (required)
        :param str compliance_type: Note: The ASPECTS_ADOPTION compliance type has been deprecated and will be decommissioned on August 31, 2025. This query parameter specifies the compliance type(s) of the listing violation counts being retrieved. See ComplianceTypeEnum for more information on the supported compliance types that can be passed in here. If more than one compliance type value is used, delimit these values with a comma. If no compliance type values are passed in, the listing count for all compliance types will be returned.  Note: Only a canned response, with counts for all listing compliance types, is returned in the Sandbox environment. Due to this limitation, the compliance_type query parameter (if used) will not have an effect on the response. 
